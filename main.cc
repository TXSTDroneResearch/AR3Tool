...
#define DISPLAY_MPLAYER 1  // Display on mplayer (depends on local decoding)
...
void* DisplayThread(void* param) {
  ...
  StreamDisplay* stream_display = nullptr;
  ...
  while (true) {
    ...
    if (ret == 0) {
      ...
      if (!stream_display) {
        stream_display = new StreamDisplay();
        bool ret = stream_display->Initialize(width, height);
        if (!ret) {
          ARSAL_PRINT(ARSAL_PRINT_ERROR, TAG,
                      "Failed to initialize StreamDisplay");
        } else {
          ARSAL_PRINT(ARSAL_PRINT_INFO, TAG, "Initialized StreamDisplay.");
        }
      }
      ...
      if (device_state->control_state == CONTROLSTATE_COMMANDING) {
        ...
        auto stats_rot = cv::format("ROT: %f %f %f", device_state->pitch,
        ...
        cv::Mat rot_matrix = ConvertEulerToMatrixYPR(
            cv::Mat(cv::Vec3d(device_state->yaw, 0, CV_PI / 2)));
        cv::Vec3d rvec;
        ...
        bool found = cv::findChessboardCorners(grayscale, board_size, image_points);
        if (found) {
      ...
      stream_display->DisplayFrame(frame.data, width * height * 3);
    }
  }

  ARSAL_PRINT(ARSAL_PRINT_DEBUG, TAG, "MPlayer thread shutting down...");
}
#endif
...
#if DISPLAY_MPLAYER
    int ret = ARSAL_Thread_Create(&device_state->mplayer_thread, DisplayThread,
                                  (void*)device_state);
    if (ret == 0) {
      ARSAL_PRINT(ARSAL_PRINT_DEBUG, TAG, "Started mplayer display thread.");
    } else {
      ARSAL_PRINT(ARSAL_PRINT_ERROR, TAG,
                  "Could not start mplayer display thread (errno %d %s).",
...
int main(int argc, char* argv[]) {
  ...
  if (device_state.stream_decoder) {
    ...
    if (device_state.mplayer_thread) {
      ARSAL_Thread_Join(device_state.mplayer_thread, nullptr);
      ARSAL_Thread_Destroy(&device_state.mplayer_thread);
    }
